...
//
//Socket descriptors
int ListenDesc, connfd; 

int main (int argc , char **argv){
	
	//varialbes
	socklen_t clientlen; // length of socket
 	int socket_desc , client_sock , c , read_size;
	struct sockaddr_un ClientAddr , serverAddr;//Structs for the client and server socket addresses.
	//
	//
	pthread_t threads[Npelates];
	int i;
	//CONNECTION BETWEEN SERVER AND CLIENT
	ListenDesc = socket( AF_LOCAL, SOCK_STREAM, 0 );
    ...
    strcpy( serverAddr.sun_path, UNIXSTR_PATH ); // Define the name of this socket
     bind( ListenDesc, ( struct sockaddr* ) &serverAddr, sizeof( serverAddr ) ); // Create the file for the socket and register it as a socket
    listen( ListenDesc, LISTENQ ); // Create request queue
       ...
       for ( ; ; ) {
         clientlen = sizeof( ClientAddr );
          // Copy next request from the queue to connfd and remove it from the queue
           connfd = accept( ListenDesc, ( struct sockaddr * ) &ClientAddr, &clientlen );
           if( connfd < 0 ) {
           ...
           pthread_create(&threads[0], NULL, (void*) orderthread, NULL);
			//Specific number of clients
		   if(op==LISTENQ-1){
			  write(connfd, &temporary, sizeof(int));
...
void *orderthread(void *dummy){
	...
	pthread_mutex_unlock(&mutex3);	
     ...
     read(connfd, &PassingNumbers[0], sizeof(int));//number of tickets
	 read(connfd, &PassingNumbers[1], sizeof(int));//Zone of theater
	 read(connfd, &PassingNumbers[2], sizeof(int));//Credit Card
	
	//credit card
	args->failed=PassingNumbers[2];
	//zone
	zone = PassingNumbers[1];
	//printf("%d",zone);
	if(zone <= 10)args->kind = "A";
	if(zone > 10 && zone <= 40)args->kind = "B";
	if(zone > 40 && zone <= 60)args->kind = "C";
	if(zone > 60 && zone <= 100)args->kind = "D";
	args->number = PassingNumbers[0];//number of seats
	//
	//printf("kind:%s  and number:%d\n",args->kind,args->number);
	//create the two threads
	pthread_create(&seatsearchthread,NULL,searchthread,args);
	pthread_create(&creditcheckthread,NULL,credithread,args);
	...
	if(Arow>=40 && Brow>=40 && Crow>=80 && Drow>=100){
		...
		//printf("args->failed orders:%d percentage\n",Forders);
		pthread_mutex_unlock(&mutex3);
		return 0;
	}
...
void *credithread(void* Dummy){
	arg_struct* args =(struct arg_struct*) Dummy;
	while(1){
